...
- (void)openSocket:(NSString *)flowNo wsSocketIP:(NSString *)ip deviceCode:(NSString *)deviceCode brand:(NSString *)brand os:(NSString *)os osVersion:(NSString *)osVersion width:(NSString *)width height:(NSString *)height {
    ...
    HJLog(@"openSocket:flowNo -> %@", flowNo);
...
//	NSString *urlStr = [NSString stringWithFormat:@"%@nudges/socket?configCode=%@&deviceCode=%@",_wsSocketURL,_flowNo, _deviceCode];
    ...
    HJLog(@"SRWebSocket  initWithURLRequest   urlStr -> %@", urlStr);
...
- (void)webSocket:(SRWebSocket *)webSocket didReceiveMessageWithString:(NSString *)string {
    ...
    if ([code isEqualToString:@"MCCM-NUDGES-SUCC-000"]) {
        [[UIApplication sharedApplication].keyWindow makeToast:@"设备绑定成功" duration:1.5f position:CSNdToastPositionCenter];
...
- (void)webSocket:(SRWebSocket *)webSocket didFailWithError:(NSError *)error {
    HJLog(@"连接失败，这里可以实现掉线自动重连，要注意以下几点");
    HJLog(@"1.判断当前网络环境，如果断网了就不要连了，等待网络到来，在发起重连");
    HJLog(@"2.判断调用层是否需要连接，例如用户都没在聊天界面，连接上去浪费流量");
    HJLog(@"3.连接次数限制，如果连接失败了，重试10次左右就可以了，不然就死循环了。或者每隔1，2，4，8，10，10秒重连...f(x) = f(x-1) * 2, (x=5)");
    [self closeSocket];
...
- (void)socketReconnect {
    ...
    if (_reconnectCounter < kRconnectCount - 1) {
        ...
        HJLog(@"Websocket Reconnected Outnumber ReconnectCount");
        if (self.reTimer) {
            [self closeRetimerSocket];
        }
        return;
