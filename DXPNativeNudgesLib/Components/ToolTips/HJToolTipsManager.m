...
#pragma mark -- 构造nudges view
	...
	// 回调
	if (_delegate && [_delegate conformsToProtocol:@protocol(ToolTipsEventDelegate)]) {
		if (_delegate && [_delegate respondsToSelector:@selector(ToolTipsShowEventByNudgesId:nudgesName:nudgesType:eventTypeId:contactId:campaignCode:batchId:source:pageName:)]) {
			[_delegate ToolTipsShowEventByNudgesId:baseModel.nudgesId nudgesName:nudgesName nudgesType:baseModel.nudgesType eventTypeId:@"" contactId:baseModel.contactId campaignCode:baseModel.campaignId batchId:@"" source:@"1" pageName:pageName];
		}
	}
